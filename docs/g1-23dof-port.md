...
验证顺序：阻尼 → START 站立 → 站立插值完成且姿态稳定 → R2+A 速度策略。**2026-09-20 起（契约 v1，观测 81 维、命令 4 维）**速度上限为前后 0.3 m/s、侧向 0.3 m/s、偏航 0.4 rad/s、俯仰 0.25 rad；右摇杆上下推（上=前倾、下=后仰）已接入第 4 命令维度。逐一验证零命令、低速、L2+B、通信丢帧、异常输出、倾角保护及恢复。记录循环与推理耗时，PC2 上应稳定满足 20 ms 周期，40 ms 控制超时会锁存阻尼。
...
| 左摇杆 | 前后/侧向速度（前后 0.3、侧向 0.3 m/s） |
| 右摇杆 | 水平轴偏航（0.4 rad/s）；垂直轴俯仰（上=前倾/下=后仰，±0.25 rad，v1 契约） |
| L2+B | 锁存阻尼并退出当前动作 |
