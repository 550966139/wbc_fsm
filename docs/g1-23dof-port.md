...
验证顺序：阻尼 → START 站立 → 站立插值完成且姿态稳定 → R2+A 速度策略。**2026-09-20 起（契约 v1，观测 81 维、命令 4 维）**速度上限为前后 0.3 m/s、侧向 0.3 m/s、偏航 0.4 rad/s。**同日晚些键位修订（操作员反馈）：俯仰指令停用**——右摇杆上下改为前后移动，与左摇杆上下叠加进 vx（同推后在 0.3 m/s 限幅处饱和）；第 4 命令维度恒为 0（训练围绕 0 对称采样，在分布内，模型无需重训）。逐一验证零命令、低速、L2+B、通信丢帧、异常输出、倾角保护及恢复。记录循环与推理耗时，PC2 上应稳定满足 20 ms 周期，40 ms 控制超时会锁存阻尼。
...
**2026-09-20 v1 重训（侧移+俯仰）Sim2Sim 实测**（新模型 `model_10000.pt`，课程扩档已删除、seed 20260920，10001 迭代无坍缩；细节见 [IMPLEMENTATION_STATUS.md](IMPLEMENTATION_STATUS.md)）：

- **左摇杆左右（侧移）可用**：A/D 各按 5 s 横移 +1.05 / −1.14 m（峰值 vy 0.37），纵向漂移 ≤0.1 m，松杆即停稳，无旧模型的"零命令 150 s 游走"病。旧结论"策略无侧移能力"已由 v1 模型取代。
- **右摇杆上下（俯仰）可用**：I/K 对应前倾/后仰，基座 ±8.5° 跟踪 ±0.25 rad 指令（误差 ≈0.10 rad 与训练指标一致），松开回中。
- 回归全过：零命令无支撑站立 10 s 零漂移（z 0.784）；W 前进 1.24 m/5 s（vx 0.27，限幅 0.3）；E 转向 −64.7°/4 s（限幅 0.4）；行进中 L2+B 即时锁存。
- 已知操作注意：FixedStand 在吊带下可能进入踝/髋 5–15 rad/s 抖振（站立 PD×吊带弹簧共振，逐次运行随机的接触盆地），会卡 R2+A 的 |dq|<1.0 门禁——遇拒绝重启仿真栈换安静盆地，或在吊带下放途中按键。非 v1 策略回归（站立控制器未改动）。
...
| 左摇杆 | 上下=前后（与右杆叠加，0.3 m/s 限幅）；左右=侧向（0.3 m/s） |
| 右摇杆 | 上下=前后（与左杆叠加，0.3 m/s 限幅）；左右=偏航（0.4 rad/s）。俯仰指令已停用（第 4 维恒 0，2026-09-20 键位修订） |
| L2+B | 锁存阻尼并退出当前动作 |
