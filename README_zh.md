...
当前程序入口仅支持 23DoF 阻尼、站立与速度策略。先阅读 [23DoF 构建与部署说明](docs/g1-23dof-port.md) 和 [实施状态](docs/IMPLEMENTATION_STATUS.md)。默认运行只做离线预检；显式 `--run` 才启动 DDS。仓库自带模型均为 29DoF，匹配的 23DoF 模型尚需提供或训练。

**下文是上游 29DoF 参考说明，旧模型、场景和运行命令不能直接用于本分支的 23DoF 真机部署。**
